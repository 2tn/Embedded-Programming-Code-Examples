...
void I2C_Read(uint8_t device, uint16_t address, uint8_t *data,
		uint32_t length) {
	I2CWriteLength = 3;
	...
	I2CMasterBuffer[2] = address;
	I2CMasterBuffer[3] = (device << 1) | 0x1;
	I2C_Engine();
}

void I2C_Write(uint8_t device, uint16_t address, uint8_t *data,
		uint32_t length) {
	...
	I2CMasterBuffer[2] = address;
	for (i = 0; i < length; i++) {
		I2CMasterBuffer[i + 3] = *data;
		data++;
	}
	I2C_Engine();
}

//24FC1025によるサンプルプログラム
int main(void) {
	I2C_Initialize();
	uint8_t WriteData[13] = "Hello, world!";
	uint8_t ReadData[13] = { 0 };
	I2C_Write(0x50, 0x00, (uint8_t *) WriteData, 13);
	int i;
	for (i = 0; i < 10000; i++)
		;
	I2C_Read(0x50, 0x00, (uint8_t *) ReadData, 13);
	printf((char *) ReadData);
	while (1) {
	}
	return 0;
}
