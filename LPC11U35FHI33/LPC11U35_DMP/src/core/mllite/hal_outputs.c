...
inv_error_t inv_generate_hal_outputs(struct inv_sensor_cal_t *sensor_cal) {
	...
	if (sensor_cal->quat.status & INV_SENSOR_ON) {
		...
		if ((sensor_cal->accel.status & sensor_cal->compass.status
				& INV_SENSOR_ON) == 0) {
			use_sensor = -1;
		}
	} else {
		if ((sensor_cal->gyro.status & sensor_cal->accel.status
				& sensor_cal->compass.status & INV_SENSOR_ON) == 0) {
			use_sensor = -1;
		}
	}
...
#define COMPASS_CONVERSION 1.52587890625e-005f
	...
	for (i = 0; i < 3; i++) {
		...
		if ((sensor_cal->compass.status & (INV_NEW_DATA | INV_RAW_DATA))
			...
			hal_out.compass_float[i] = inv_biquad_filter_process(
					&hal_out.lp_filter[i],
					(float) compass[i]) * COMPASS_CONVERSION;

		} else if ((sensor_cal->compass.status & INV_NEW_DATA)
				== INV_NEW_DATA) {
...
float compass_low_pass_filter_coeff[5] = { +2.000000000000f, +1.000000000000f,
		-1.279632424998f, +0.477592250073f, +0.049489956269f };
...
inv_error_t inv_init_hal_outputs(void) {
	...
	for (i = 0; i < 3; i++) {
		inv_init_biquad_filter(&hal_out.lp_filter[i],
