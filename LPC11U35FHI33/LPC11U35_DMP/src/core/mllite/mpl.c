...
inv_error_t inv_init_mpl(void) {
	...
	INV_ERROR_CHECK (inv_init_start_manager()); /* initialize the data builder */
INV_ERROR_CHECK	(inv_init_data_builder());INV_ERROR_CHECK
	(inv_enable_results_holder());return
INV_SUCCESS	;
}
...
inv_error_t inv_start_mpl(void) {
	INV_ERROR_CHECK (inv_execute_mpl_start_notification());return INV_SUCCESS;
}
