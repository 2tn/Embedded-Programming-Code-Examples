...
float inv_compass_angle(const long *compass, const long *grav,
	...
	cgcross[0] = 0L;
	cgcross[1] = inv_q30_mult(compass[1], grav[2])
			- inv_q30_mult(compass[2], grav[1]);
	cgcross[2] = inv_q30_mult(compass[2], grav[0])
			- inv_q30_mult(compass[0], grav[2]);
	cgcross[3] = inv_q30_mult(compass[0], grav[1])
			- inv_q30_mult(compass[1], grav[0]);
...
void inv_quaternion_to_rotation(const long *quat, long *rot) {
	rot[0] = inv_q29_mult(quat[1], quat[1]) + inv_q29_mult(quat[0], quat[0])
			- 1073741824L;
	...
	rot[4] = inv_q29_mult(quat[2], quat[2]) + inv_q29_mult(quat[0], quat[0])
			- 1073741824L;
	...
	rot[8] = inv_q29_mult(quat[3], quat[3]) + inv_q29_mult(quat[0], quat[0])
			- 1073741824L;
}
...
float inv_matrix_det(float *p, int *n) {
	...
	for (i = 0, j = 0; j < m; j++) {
		*n = m;
		...
		sum = sum
				+ *(p + 6 * i + j) * SIGNM(i + j) * inv_matrix_det(&d[0][0], n);
	}
 ...
 */
void inv_convert_to_body(unsigned short orientation, const long *input,
		long *output) {
 ...
 * @param[out] output Output vector, length 3
 */
void inv_convert_to_chip(unsigned short orientation, const long *input,
		long *output) {
...
void inv_convert_to_body_with_scale(unsigned short orientation,
		long sensitivity, const long *input, long *output) {
	output[0] = inv_q30_mult(
			input[orientation & 0x03] * SIGNSET(orientation & 0x004),
			sensitivity);
	output[1] = inv_q30_mult(
			input[(orientation >> 3) & 0x03] * SIGNSET(orientation & 0x020),
			sensitivity);
	output[2] = inv_q30_mult(
			input[(orientation >> 6) & 0x03] * SIGNSET(orientation & 0x100),
			sensitivity);
}
...
void inv_get_cross_product_vec(float *cgcross, float compass[3],
		float grav[3]) {
