...
#define SSPxCLKDIV	LPC_SYSCON->SSP0CLKDIV
#define	SSPxDR		LPC_SSP0->DR
#define	SSPxSR		LPC_SSP0->SR
#define	SSPxCR0		LPC_SSP0->CR0
#define	SSPxCR1		LPC_SSP0->CR1
#define	SSPxCPSR	LPC_SSP0->CPSR
#define	CS_LOW()	{LPC_GPIO1->DATA &= ~(1<<4);}	/* Set P1.4 low */
#define	CS_HIGH()	{LPC_GPIO1->DATA |= (1<<4);}	/* Set P1.4 high */
...
{
#if SSP_CH == 0
	LPC_GPIO1->DIR |= (1 << 4); /* CS# (P1_4) = output */
	LPC_SYSCON->SYSAHBCLKCTRL |= (0x1 << 11);
	SSPxCLKDIV = 1; /* PCLK = sysclk */
	LPC_SYSCON->PRESETCTRL |= (0x1 << 0);
	SSPxCPSR = 0x02; /* fc=PCLK/2 */
...
DWORD get_fattime(void) {
	...
	return ((DWORD)(year - 1980) << 25) | ((DWORD) month << 21)
			| ((DWORD) mday << 16) | ((DWORD) hour << 11) | ((DWORD) min << 5)
			| ((DWORD) sec >> 1);
}
